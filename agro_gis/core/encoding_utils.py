...
def _text_quality(text: str) -> float:
    ...
    alnum_ratio = sum(1 for ch in text if ch.isalnum()) / length
    ...
    suspect_punct_ratio = sum(1 for ch in text if "‘" <= ch <= "‟") / length
    score = printable_ratio - control_ratio + (0.35 * cyr_ratio) + (0.15 * alnum_ratio) - (0.25 * suspect_punct_ratio)
    if length >= 8 and alnum_ratio < 0.25:
        score -= 0.35
...
def _looks_like_binary_text(text: str) -> bool:
    ...
    length = len(text)
    control_ratio = sum(1 for ch in text if ord(ch) < 32 and ch not in "\n\r\t") / length
    has_delimiters = any(marker in text for marker in ("\n", ",", ";", "{", "}", "(", ")"))
    return control_ratio > 0.05 and not has_delimiters
...
def detect_encoding(data: bytes) -> tuple[str, float]:
    ...
    if best is None or not best.encoding:
        ...
        detector_confidence = max(0.0, min(1.0, detector_confidence))
        logger.debug("Detected encoding=%s confidence=%.3f", detector_encoding, detector_confidence)
        if detector_encoding in SUPPORTED_DETECTED_ENCODINGS:
    ...
    if best_encoding is None or best_score < 0.72:
...
def read_text(path: str | Path, forced_encoding: str | None = None) -> tuple[str, str, float]:
    ...
    if forced_encoding:
        normalized = forced_encoding.lower().replace("-", "")
        if normalized == "utf16":
            forced_encoding = "utf-16"
        try:
        ...
        if _looks_like_binary_text(text):
            raise EncodingDetectionError("Decoded text appears binary/corrupted for selected encoding")
        return text, forced_encoding, 1.0
    ...
    text = text.lstrip("\ufeff")
    if _looks_like_binary_text(text):
        raise EncodingDetectionError("Decoded text appears binary/corrupted; try another encoding")
